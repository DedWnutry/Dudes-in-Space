...
namespace Star_Ship
{
    public class ShipComponent : MonoBehaviour, IShipComponent, IShipComponentInformation
    {
        ...
        private void SetupTaskScreen()
        {
            ...
            _taskScreen = Instantiate(_taskScreenPrefab, _healthSlider.transform.GetChild(0).transform.position, _healthSlider.transform.GetChild(0).transform.rotation, _healthSlider.transform.GetChild(0).transform);

            _taskScreen.SetActive(true);

            _taskScreen.GetComponent<MinigameMain>().coundownSlider.maxValue = GetCurrentTask().Time;

            _taskScreen.GetComponent<MinigameMain>().coundownSlider.value = GetCurrentTask().Time;
        }
        ...
        private IEnumerator FixCycle(int solvingSpeed, IAstronaut astronaut)
        {
            ...
            while (TaskSolvingProgress <= 1f && _astronaut.IsOccupied == true)
            {
                ...
                _taskScreen.GetComponent<MinigameMain>().coundownSlider.value--;

                yield return new WaitForSeconds(1);
            }

            if (_taskScreen.GetComponent<MinigameMain>().solved == true) TryFixShipComponent();
            else 
            {
